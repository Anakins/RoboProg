...
|---|---|
| **Commande de retour à la maison** | Combinée à un **capteur pluie** configuré, elle permet de **rappeler activement le robot** dès que la pluie est détectée (voir section 4). Elle n'intervient pas dans la relance automatique du cycle classique après bordures (section 5), qui repose uniquement sur Statut + valeur "à la maison" enregistrée. Si vous ne voulez pas du tout gérer la pluie, laissez simplement le capteur pluie et/ou cette commande vides : sans les deux ensemble, le robot ne sera jamais rappelé automatiquement. |
...
- **Statut** seul (sans retour à la maison) : permet d'afficher l'état du robot, et suffit à lui seul pour activer la relance automatique après bordures (avec la valeur "à la maison" enregistrée) — RoboProg n'a en revanche aucun moyen de faire rentrer le robot de force sans la commande de retour à la maison (utile uniquement pour la pluie).
- **Statut + valeur "à la maison" enregistrée** : suffisant pour la relance automatique après bordures (section 5) — la commande de retour à la maison n'est pas nécessaire pour cette fonctionnalité, elle ne sert qu'au rappel actif en cas de pluie.
...
- **Relance automatique du cycle classique** (case à cocher) : après **n'importe quelle** coupe de bordures — rattrapage **ou** jour normal programmé — RoboProg peut relancer automatiquement la tonte classique une fois le robot de retour à la maison. Elle n'est plus liée à la case rattrapage : cochez-la même si vous voulez juste enchaîner bordures → tonte classique un jour de bordures ordinaire.
  ...
  - Nécessite la commande Statut, et d'avoir enregistré la valeur "à la maison" (bouton dédié dans le formulaire). La commande de retour à la maison n'est pas requise pour cette fonctionnalité : elle ne sert qu'au rappel actif en cas de pluie (voir section 2).
