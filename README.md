...
- **Statut + valeur "à la maison" enregistrée** : nécessaire pour la relance automatique après bordures (section 5) — la commande de retour à la maison n'est pas nécessaire pour cette fonctionnalité, elle ne sert qu'au rappel actif en cas de pluie.
- **Bordures** sans **Statut** : la coupe des bordures elle-même fonctionne (jours fixes ou intervalle, rattrapage), mais l'option de relance automatique du cycle classique après le rattrapage reste indisponible (case grisée/refusée).
- **Bordures** avec **Statut** (+ valeur "à la maison" enregistrée) : en plus de la coupe des bordures, l'option de relance automatique du cycle classique après bordures devient disponible (section 5).
...
Commande Jeedom ou heure fixe pour le début/la fin au format `HMM`/`HHMM`), marge de sécurité avant l'heure de fin, et espacement en jours entre deux tontes classiques.
...
- **Relance automatique du cycle classique** (case à cocher) : après **n'importe quelle** coupe de bordures — rattrapage **ou** jour normal programmé — RoboProg peut relancer automatiquement la tonte classique une fois le robot de retour à la maison. Elle n'est plus liée à la case rattrapage : cochez-la même si vous voulez juste enchaîner bordures → tonte classique un jour de bordures ordinaire.
  - **Nécessite la commande Statut, ainsi que d'avoir enregistré la valeur de statut correspondant au moment où le robot est sur sa base** (bouton dédié dans le formulaire) : sans ces deux éléments, cette option ne peut pas être activée (voir section 2).
...
- **Pas de tonte** (cochée par défaut) : envoyée en fin de fenêtre horaire si aucune tonte n'a pu avoir lieu (ni classique, ni bordures). Précise systématiquement la date de la prochaine tonte prévue, avec son **type entre parenthèses** — `(TONTE)`, `(BORDURES)` ou `(BORDURES + TONTE)` — puisque cette prochaine échéance peut tout aussi bien être une tonte classique qu'un jour de bordures.
- **Erreur** (décochée par défaut) : envoyée si une même erreur (valeur de la commande Erreur) persiste sans interruption pendant **3 minutes** — le temps que le robot ait une vraie chance de se débloquer tout seul avant d'alerter inutilement. Le message commence par ⚠️ suivi du libellé exact de l'erreur. Une seule notification par occurrence : si l'erreur disparaît puis revient (même libellé ou non), un nouveau décompte de 3 minutes redémarre avant de renotifier. Case grisée/décochée automatiquement si aucune commande Erreur n'est liée, puisqu'aucune notification de ce type ne peut alors être envoyée.
